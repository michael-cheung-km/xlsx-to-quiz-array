    ...
    for x in range(0, len(wb.worksheets)):
        for i in range(0, len(pullKeywords(wb, x))):
            ...
            if not os.path.exists("PARSED"):
                os.makedirs("PARSED")
            if not os.path.exists("PARSED/" + str(sortedArr[0][0][:-3])):
                os.makedirs("PARSED/" + str(sortedArr[0][0][:-3]))
            fileNameStr = "PARSED/" + str(sortedArr[0][0][:-3]) + '/' + str(sortedArr[0][0][:-3]) + ".js"
            fileHTMLNameStr = "PARSED/" + str(sortedArr[0][0][:-3]) + '/' + 'quiz' + ".php"
            print('PARSING: ' + fileNameStr.replace("PARSED/", ""))
            jsStr = processKeywordQsArr(pullKeywordQsArr(wb, x, pullKeywords(wb, x)[i]))
            htmlStr = '<script src="' + str(sortedArr[0][0][:-3]) + '.js"></script>'
            f = open(fileNameStr, "w")
            f1 = open('template/js/head.txt', "r")
            f2 = open('template/js/foot.txt', "r")
            f3 = open(fileHTMLNameStr, "w")
            f4 = open('template/html/head.txt', "r")
            f5 = open('template/html/foot.txt', "r")
            f.write(f1.read() + '\n')
            f.write(jsStr)
            f.write('\n\n' + f2.read())
            f3.write(f4.read() + '\n')
